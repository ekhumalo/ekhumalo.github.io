...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        ...
        for (int i = 1; i < segment.size(); i++) {
            Road road = _map.getLabel(segment.get(i - 1), segment.get(i));
            String roadName = road.toString();
            Direction newDir = road.direction();

            if (currDir == newDir && roadName.equals(lastRoad)) {
                roadLength = roadLength + road.length();
            } else {
                if (i > 1) {
                    System.out.printf(newLine + "%d. Take %s %s for %.1f miles",
                            seq, lastRoad, currDir.fullName(), roadLength);
                    seq = seq + 1;
                    newLine = ".%n";
                }
                lastRoad = roadName;
        ...
        System.out.printf(newLine + "%d. Take %s %s for %.1f miles to %s.%n",
                seq, lastRoad, currDir.fullName(),
                roadLength, _map.getLabel(from));
        return seq + 1;
    }
    ...
    private static class RoadMap extends LabeledGraph<Location, Road> {
        /** An empty RoadMap. */
        RoadMap() {
            super(new LabeledGraph<Location, Road>(new DirectedGraph()));
        }
    }
