 ...
 *  each distinct Move.
 *  @author P. N. Hilfinger & E. Khumalo.
 */
class Move {
    ...
    boolean isExtend() {
        double extend = Math.hypot((col1() - col0()), row1() - row0());
        return extend >= 1 && extend <= Math.sqrt(2);
    }

    /** Return true if thIs is a jump (move to adjacent square). */
    boolean isJump() {
        double jump = Math.hypot((col1() - col0()), row1() - row0());
        return jump >= 2 && jump <= 2 * Math.sqrt(2);
    }
    ...
    /**
     * Add index to ChangeSquares Array.
     * @param square index.
     */
    void addChange(int square) {
        if (!_changedSquares.contains(square)) {
            _changedSquares.add(square);
        }
    }

    /**
     * All changed neighbors.
     * @return List.
     */
    ArrayList<Integer> getChangedSquares() {
        return _changedSquares;
    }
     ...
     * Changed Squares.
     */
    private ArrayList<Integer> _changedSquares = new ArrayList<Integer>();
